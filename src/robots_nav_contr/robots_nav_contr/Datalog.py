...
def OdometryDataLog(OdometryData):
    ...
    try:
        ...
        # Create the table if it doesn't exist
        table_name = f"{'Odometry'}_{date.today().strftime('%Y_%m_%d')}"
        # Log the table name
        SQL_Logs.info(f"Table name: {table_name} Generated")
        # Create a table with the current date in the name
        if table_name:
            SQL_Logs.info(f"Creating table {table_name}...")
            ...
            cursor.execute(f'''
                ...
                BEGIN
                    CREATE TABLE [{table_name}] (
                        ...
                        Theta VARCHAR(50),
                        robotID VARCHAR(50),
                        LeftSpeed VARCHAR(50),
                        RightSpeed VARCHAR(50),
                        timestamp DATETIME DEFAULT GETDATE()
                    )
                END
                ''')
        ...
        # Insert sensor data into the table
        cursor.execute(
            f"INSERT INTO {table_name} ( X, Y, Theta, robotID, LeftSpeed, RightSpeed, timestamp) VALUES ( ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)"
            , OdometryData)
...
if __name__ == "__main__":
    ...
    Odo_data = [252, 2548, 548, 5554, 545,  ]

    # Call the function to connect to the database
    RobotDataLog(sensor_data)
    OdometryDataLog(Odo_data)